...
import static edu.wpi.first.hal.simulation.EncoderDataJNI.getDistance;
import static frc.lib.constants.RobotConstants.GeneralConstants.intakePoses;
import static frc.lib.util.GeometryUtil.toTransform2d;
...
public class ToIntakePoseCommand extends Command {
    ...
    public void initialize() {
        ...
        for (int i =0; i < intakePoses.length; i++) {
            double distance = GeometryUtil.toTransform2d(drive.getPose()).getTranslation().getDistance(GeometryUtil.toTransform2d(intakePoses[i]).getTranslation());
